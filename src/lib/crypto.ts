...
export function encryptJson(value: unknown): { data: Uint8Array; iv: Uint8Array } {
  const iv = crypto.randomBytes(12)
  const cipher = crypto.createCipheriv('aes-256-gcm', KEY, iv)
...
export function decryptJson<T>(data: Buffer | Uint8Array, iv: Buffer | Uint8Array): T {
  const d = Buffer.isBuffer(data) ? data : Buffer.from(data)
  const i = Buffer.isBuffer(iv) ? iv : Buffer.from(iv)
  const decipher = crypto.createDecipheriv('aes-256-gcm', KEY, i)
  decipher.setAuthTag(d.subarray(d.length - 16))
...
export function encryptString(value: string): { data: Uint8Array; iv: Uint8Array } {
  return encryptJson({ value })
}

export function decryptString(data: Buffer | Uint8Array, iv: Buffer | Uint8Array): string {
  const parsed = decryptJson<{ value: string }>(data, iv)
  return parsed.value
}
